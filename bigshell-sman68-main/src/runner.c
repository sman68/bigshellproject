...
expand_command_words(struct command *cmd)
{
  for (size_t i = 0; i < cmd->word_count; ++i) {
   ...
   /* TODO Assignment values - IMPLEMENTED */
   /* TODO I/O Filenames - IMPLEMENTED */
...
do_variable_assignment(struct command const *cmd, int export_all)
{
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    /* TODO Assign - IMPLEMENTED*/
    /* TODO Export (if export_all != 0) - IMPLEMENTED*/
...
get_io_flags(enum io_operator io_op)
{
  ...
  switch (io_op) {
    case OP_LESSAND: /* <& */
    case OP_LESS:    /* < */
      flags = O_RDONLY;     /* TODO - IMPLEMENTED */
      break;
    case OP_GREATAND: /* >& */
    case OP_GREAT:    /* > */
      flags = O_WRONLY | O_CREAT | O_EXCL;      /* TODO - IMPLEMENTED*/
      break;
    case OP_DGREAT: /* >> */
      flags = O_WRONLY | O_CREAT | O_APPEND;    /* TODO - IMPLEMENTED */
      break;
    case OP_LESSGREAT: /* <> */
      flags = O_RDWR | O_CREAT;       /* TODO - IMPLEMENTED*/
      break;
    case OP_CLOBBER: /* >| */
      flags = O_WRONLY | O_CREAT | O_TRUNC;     /* TODO - IMPLEMENTED*/
      break;
  }
  return flags;
}
...
move_fd(int src, int dst)
{
  ...
  /* TODO move src to dst - IMPLEMENTED*/
  /* TODO close src - IMPLEMENTED*/
...
do_builtin_io_redirects(struct command *cmd, struct builtin_redir **redir_list)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    file_open:;
      ...
      /* TODO Open the specified file. -IMPLEMENTED */
...
do_io_redirects(struct command *cmd)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    if (r->io_op == OP_GREATAND || r->io_op == OP_LESSAND) {
      ...
      if (strcmp(r->filename, "-") == 0) {
        /* [n]>&- and [n]<&- close file descriptor [n] */
        /* TODO close file descriptor n. - IMPLEMENTED
         *
        ...
        if (*(r->filename) && !*end /* <--- this is part of the strtol idiom */
          ...
          /* TODO duplicate src to dst. - IMPLEMENTED */
...
run_command_list(struct command_list *cl) {
    ...
    int pipeline_fds[2] = {-1, -1};  // Use new pipeline management variables
    ...
    for (size_t i = 0; i < cl->command_count; ++i) {
        ...
        if (is_pl) {
            if (pipe(pipeline_fds) < 0) goto err;
        } else {
            pipeline_fds[0] = -1;
            pipeline_fds[1] = -1;
        }
        ...
        if (!builtin || (builtin && !is_fg)) {
         ...
         /* All of the processes in a pipeline (or single command) belong to the
             ...
             *
             * Initially pipeline_data.pgid is set to 0 (unset). We will assign the
             * first command in a pipeline to a new process group, then store that pgid
             * for later use.
             *
         ...
         * operator */
        if (is_fg) {
            if (wait_on_fg_pgid(pipeline_pgid) < 0) {
                warn(0);
                params.status = 127;
                return -1;
            }
